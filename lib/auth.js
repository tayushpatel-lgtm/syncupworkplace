...
/** For pages: sends people to login, then to change a first-time/reset password, then through onboarding. */
export async function requireUser({ skipOnboardingGate = false, skipPasswordGate = false } = {}) {
  const user = await currentUser();
  if (!user) redirect('/login');
  if (!skipPasswordGate && user.mustChangePassword) redirect('/change-password');
