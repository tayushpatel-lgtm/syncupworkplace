...
export function parseRepeat(value) {
  const raw = String(value || 'NONE').trim().toUpperCase().replace(/[_-]+/g, ' ').replace(/\s+/g, ' ');
  const compact = raw.replace(/ /g, '');
  if (TASK_REPEATS.includes(raw)) return raw;
  if (TASK_REPEATS.includes(compact)) return compact;
  const aliases = {
    DAY: 'DAILY',
    DAYS: 'DAILY',
    EVERYDAY: 'DAILY',
  ...
  return aliases[raw] || aliases[compact] || 'NONE';
}
...
export function parseRepeatInput(body, { todayKey } = {}) {
  ...
  let dueDate = parseYmd(body.dueDate || body.due || body.deadline);
