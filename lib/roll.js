...
export async function dayRoll(key) {
  ...
  for (const s of sessions) {
    ...
    const raw = (end - s.startedAt.getTime()) / 60000;
    const mins = Number.isFinite(raw) ? Math.max(0, raw) : 0;
...
export async function buildEodSummary(key) {
  ...
  if (!working) {
    return { working: false, holidayName, date: key, present: [], absent: [], absentUsers: [], notPickedUp: [] };
  }

  const present = rows.filter((r) => PRESENT_STATES.has(r.state)).map((r) => r.name);
  const absentRows = rows.filter((r) => r.state === 'not-in');
  const absent = absentRows.map((r) => r.name);
  const absentUsers = absentRows.length
    ? await prisma.user.findMany({
        where: { id: { in: absentRows.map((r) => r.id) } },
  ...
  return { working: true, holidayName, date: key, present, absent, absentUsers, notPickedUp };
}
