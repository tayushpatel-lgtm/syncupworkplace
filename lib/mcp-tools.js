...
import { postToSlack, taskAssignedMessage, statusChangeMessage, sendDirectMessage, taskAssignedDm } from './slack';
...
export async function runTool(name, args = {}, ctx = {}) {
  switch (name) {
    ...
    case 'assign_task': {
      ...
      await sendDirectMessage(assignee, taskAssignedDm(task, ctx.actor));
    ...
    case 'update_task_status': {
      ...
      const matches = await prisma.task.findMany({ where, include: { assignee: { select: { name: true } } } });
      ...
      if (task.status !== args.status) {
        await postToSlack('status', statusChangeMessage(updated, { name: task.assignee.name }, task.status, args.status));
      }
