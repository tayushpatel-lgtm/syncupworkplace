...
import { openTaskCount, buildPlan, maybeSpawnOnComplete } from './day';
import { parseRepeatInput, TASK_REPEATS } from './recurrence';
import { postToSlack, taskAssignedMessage, statusChangeMessage, statusChangeDm, sendDirectMessage, taskAssignedDm } from './slack';
import { dayKey, dayDate, dateFieldKey, shiftDay, formatDuration, rangeKeys } from './dates';
...
export const WRITE_TOOLS = [
  {
    name: 'assign_task',
    description:
      'Create a task and assign it to someone. One-off tasks land on their plan today; repeating ones show on the day they are due. Completing a repeating task opens the next occurrence. Subject to the ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        dueDate: DATE,
        due: { type: 'string', description: 'Alias for dueDate, YYYY-MM-DD.' },
        repeat: {
          type: 'string',
          enum: TASK_REPEATS,
          description:
            'How often it comes back after it is completed: NONE (one-off), DAILY, WEEKDAYS (Monday to Saturday), WEEKLY, MONTHLY, YEARLY. Completing a repeating task creates the next occurrence. If you omit dueDate, the first occurrence is due today.',
        },
        repeatUntil: { type: 'string', description: 'Last day an occurrence may be due, YYYY-MM-DD. Omit to never end.' },
        repeatWeekdays: {
          type: 'array',
          items: { type: 'number' },
          description: 'ISO weekdays 1 (Mon) through 7 (Sun). Used when repeat is WEEKLY.',
        },
        repeatInterval: { type: 'number', description: 'Every N days/weeks/months/years. Defaults to 1.' },
        repeatCount: { type: 'number', description: 'Stop after this many occurrences, counting the first.' },
      },
    },
  },
  {
    ...
    description:
      ...
      'For a repeating series, this picks the current open occurrence when several share a title, and completing it opens the next one. ' +
      'Requires a read-write token.',
    inputSchema: {
      type: 'object',
...
function parseStatus(value) {
  ...
  const aliases = { DONE: 'COMPLETED', COMPLETE: 'COMPLETED', IN_PROGRESS: 'PROGRESS' };
  const mapped = aliases[raw] || raw;
  return STATUSES.includes(mapped) ? mapped : null;
}

/**
 * Match a task by title (and optional assignee). When a repeating series has
 * both completed and open rows with the same title, pick the unique open one.
 */
async function findTask(args) {
  const title = String(args.task || '').trim();
  ...
  if (matches.length === 0) return { error: `No task matches "${title}".` };
  if (matches.length === 1) return { task: matches[0] };

  const open = matches.filter((t) => t.status === 'PENDING' || t.status === 'PROGRESS' || t.status === 'BLOCKED');
  if (open.length === 1) return { task: open[0] };

  return {
    error: `More than one task matches "${title}": ${matches
      .map((t) => `${t.title} (${t.assignee.name}, ${t.status.toLowerCase()})`)
      .join('; ')}. Be more specific, or add "assignee".`,
  };
}
...
export async function runTool(name, args = {}, ctx = {}) {
  switch (name) {
    ...
    case 'list_tasks': {
      const where = {};
      const statusArg = String(args.status || '').trim();
      if (statusArg.toUpperCase() === 'OPEN') where.status = { in: ['PENDING', 'PROGRESS'] };
      else if (statusArg) {
    ...
    case 'leave_overview': {
      const people = await prisma.user.findMany({
        where: { active: true },
        select: { id: true, name: true, department: true, employmentType: true, casualLeaveBalance: true, sickLeaveBalance: true },
        orderBy: { name: 'asc' },
      });
    ...
    case 'assign_task': {
      ...
      const recurrence = parseRepeatInput(
        { ...args, dueDate: args.dueDate || args.due || args.deadline },
        { todayKey: dayKey() },
      );
      if (
        ...
        return { error: 'The end date has to be on or after the first deadline.' };
      }
      ...
      return {
        ...
        repeat: recurrence.repeat,
        dueDate: recurrence.dueDate ? dateFieldKey(recurrence.dueDate) : null,
      };
    }
    ...
    case 'update_task_status': {
      ...
      const found = await findTask(args);
      if (found.error) return { error: found.error };
      ...
      if (task.status !== status) {
        await postToSlack('status', statusChangeMessage(updated, task.assignee, task.status, status));
        await sendDirectMessage('status', task.assignee, statusChangeDm(updated, task.status, status));
      }

      const next = await maybeSpawnOnComplete(task, updated);
      if (next) {
        const settings = await getSettings();
        const assignee = await prisma.user.findUnique({ where: { id: next.assigneeId } });
        if (assignee) await buildPlan(assignee, dayKey(), settings);
      }
      ...
      return {
        ...
        to: status,
        nextOccurrence: next
          ? { id: next.id, title: next.title, dueDate: next.dueDate ? dateFieldKey(next.dueDate) : null }
          : null,
      };
    }
    ...
    case 'decide_leave': {
      ...
      if (args.decision === 'APPROVED') {
        const field = pending.kind === 'SICK' ? 'sickLeaveBalance' : 'casualLeaveBalance';
        const person = await prisma.user.findUnique({ where: { id: pending.userId }, select: { [field]: true } });
        if (person) {
          await prisma.user.update({
            where: { id: pending.userId },
            data: { [field]: Math.max(0, person[field] - pending.days) },
          });
        }
      }
