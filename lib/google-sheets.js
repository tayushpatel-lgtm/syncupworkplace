...
const TABLES = [
  {
    title: 'Users',
    headers: [
      'id', 'name', 'email', 'role', 'employmentType', 'department', 'title', 'active', 'joinedAt',
      'casualLeaveBalance', 'sickLeaveBalance',
    ],
    rows: () =>
      prisma.user.findMany({
        select: {
          id: true, name: true, email: true, role: true, employmentType: true, department: true,
          title: true, active: true, joinedAt: true, casualLeaveBalance: true, sickLeaveBalance: true,
        },
        orderBy: { name: 'asc' },
      }),
  },
  ...
  },
  {
    title: 'Tasks',
    headers: [
      'id', 'title', 'status', 'priority', 'dueDate', 'repeat', 'repeatUntil', 'repeatInterval', 'repeatCount', 'seriesId',
      'assigneeId', 'creatorId', 'completedAt', 'createdAt',
    ],
...
export async function syncAllToSheets() {
  ...
  try {
    ...
    // One tab at a time was well over a minute for ~14 tables — each is an
    // independent range, so there's nothing stopping them running at once.
    await Promise.all(
