...
export function checkedOutInactiveDm(minutesWorked, idleAfterMinutes) {
  const hours = (minutesWorked / 60).toFixed(1);
  return {
    text: 'You were checked out automatically',
    blocks: [
      {
        type: 'section',
        text: {
          type: 'mrkdwn',
          text: `:crescent_moon: *You were checked out automatically* — no activity was detected for ${idleAfterMinutes} minutes.\n${hours}h recorded before that.`,
        },
      },
    ],
  };
...
export function checkInDm(late) {
  return {
    text: `You checked in${late ? ' (late)' : ''}`,
    blocks: [
      {
        type: 'context',
        elements: [{ type: 'mrkdwn', text: `${late ? ':warning:' : ':white_check_mark:'} *You checked in*${late ? ' — late' : ''}` }],
      },
    ],
  };
}

export function checkOutDm(minutesWorked) {
  const hours = (minutesWorked / 60).toFixed(1);
  return {
    ...
    blocks: [
      {
        type: 'context',
        elements: [{ type: 'mrkdwn', text: `:door: *You checked out* · ${hours}h recorded today` }],
      },
    ],
  };
}
