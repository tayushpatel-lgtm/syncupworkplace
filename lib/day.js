...
import { publicAppUrl } from './origin';
import {
  postChannelEvent,
  checkInMessage,
  checkOutMessage,
...
import { belongsOnTodayPlan, nextDueDate, parseRepeat } from './recurrence';
...
/// A live timer is never closed just because the clock hit midnight. It keeps
/// running until the person stops it, or recorded work in this continuous
/// stretch crosses 12 hours (tab left open, forgotten).
export const MAX_CONTINUOUS_WORK_MINUTES = 12 * 60;
...
function workCapEnd(startedAt, priorWorkMinutes) {
  const remainingMin = MAX_CONTINUOUS_WORK_MINUTES - priorWorkMinutes;
  return new Date(startedAt.getTime() + remainingMin * 60 * 1000);
}

/**
 * Work minutes in the closed sessions that abut this one (midnight splits,
 * work → break → work). Used so a stretch that crossed midnight still caps
 * at 12 hours of recorded work, not 12 hours from the continuation's start.
 */
async function abuttingPriorWorkMinutes(db, userId, session, opts = {}) {
  const lookback = new Date(session.startedAt.getTime() - (MAX_CONTINUOUS_WORK_MINUTES + 60) * 60 * 1000);
  const recent = await db.workSession.findMany({
    where: {
      userId,
      id: { not: session.id },
  ...
  for (const prev of recent) {
    if (Math.abs(prev.endedAt.getTime() - cursor) > 2000) break;
    if (prev.kind === 'WORK') {
      const skip = opts.excludeDateKey && dateFieldKey(prev.date) === opts.excludeDateKey;
      if (!skip) minutes += closedMinutes(prev.startedAt, prev.endedAt);
    }
    cursor = prev.startedAt.getTime();
  }
  return minutes;
}

async function ensureTodayAttendance(tx, userId, todayKey, settings) {
  const date = dayDate(todayKey);
  const existing = await tx.attendance.findUnique({
    where: { userId_date: { userId, date } },
  });
  if (existing?.checkInAt) return false;
  const person = await tx.user.findUnique({ where: { id: userId }, select: { checkInBy: true } });
  const deadline = checkInDeadline(person || {}, settings);
  const checkInAt = startOfDay(todayKey);
  const data = { checkInAt, checkInBy: deadline, late: false, status: 'PRESENT' };
  if (existing) {
    await tx.attendance.update({ where: { id: existing.id }, data });
  } else {
    await tx.attendance.create({ data: { userId, date, ...data } });
  }
  return true;
}
...
export async function reconcileSessions(userId, settings) {
  ...
  await prisma.$transaction(async (tx) => {
    for (const session of open) {
      ...
      const priorWork =
        session.kind === 'WORK' ? await abuttingPriorWorkMinutes(tx, userId, session) : 0;
      const liveWork =
        session.kind === 'WORK' ? (now.getTime() - session.startedAt.getTime()) / 60000 : 0;
      const overCap = session.kind === 'WORK' && priorWork + liveWork >= MAX_CONTINUOUS_WORK_MINUTES;
      if (!needs && !overCap) continue;

      const startKey = dateFieldKey(session.date);
      const beatKey = dayKey(beat);
      const cutoff = cutoffMinutes(session, cfg);
      const capEnd = overCap ? workCapEnd(session.startedAt, priorWork) : null;
      closed += 1;
      ...
      if (overCap && capEnd && !stale) {
        const splitAt = endOfDay(startKey);
        if (!fromPastDay || capEnd.getTime() <= splitAt.getTime()) {
          const endedAt = new Date(
            Math.min(now.getTime(), Math.max(session.startedAt.getTime(), capEnd.getTime())),
          );
          await tx.workSession.update({
            where: { id: session.id },
            data: { endedAt, openUserId: null },
          });
          continue;
        }
      }
      ...
      if (liveAcrossMidnight || beatOnLaterDay) {
        const splitAt = endOfDay(startKey);
        let originalEnd = beatOnLaterDay ? splitAt : (splitAt.getTime() < now.getTime() ? splitAt : beat);
        if (!stale && capEnd && capEnd.getTime() < originalEnd.getTime()) {
          originalEnd = new Date(Math.max(session.startedAt.getTime(), capEnd.getTime()));
          await tx.workSession.update({
            where: { id: session.id },
        ...
        let leaveOpen = !stale && !overCap && contKey === todayKey;
        let contEnd = leaveOpen ? null : beat;
        if (!stale && overCap && capEnd && capEnd.getTime() > contStart.getTime()) {
          leaveOpen = false;
          contEnd = new Date(Math.min(now.getTime(), capEnd.getTime()));
        }
        ...
        if (contKey === todayKey && !stale) {
          const stamped = await ensureTodayAttendance(tx, userId, todayKey, cfg);
          if (stamped) carryPlan = true;
        }
  ...
  if (carryPlan) {
    await buildPlan({ id: userId }, todayKey, cfg);
  }
    ...
    if (user) {
      if (inactiveClose) {
        ...
        await sendDirectMessage(
          'inactive',
          user,
          checkedOutInactiveDm(totals.work, cfg.idleAfterMinutes, `${publicAppUrl()}/`),
          cfg,
        );
      }
      if (staleBreakAlert) {
...
export async function dayTotals(userId, key) {
  ...
  if (totals.running) {
    ...
    if (open) {
      // Only earlier-day abutting work (a midnight split). Same-day closed
      // work is already in totals.work — counting it again doubles the clock
      // the moment someone takes a break or comes back from one.
      totals.priorWork = Math.round(
        await abuttingPriorWorkMinutes(prisma, userId, open, { excludeDateKey: key }),
      );
    }
  }
  return totals;
 ...
 * Live timers that have crossed 12 hours of recorded work are stopped here too.
 */
export async function heartbeat(userId) {
  const settings = await getSettings();
  const now = new Date();
  const open = await prisma.workSession.findMany({ where: { userId, endedAt: null } });
  if (open.length === 0) return { reconciled: false, running: false };

  let dirty = open.some((session) => sessionNeedsReconcile(session, now, settings).needs);
  if (!dirty) {
    for (const session of open) {
      if (session.kind !== 'WORK') continue;
      const prior = await abuttingPriorWorkMinutes(prisma, userId, session);
      const live = (now.getTime() - session.startedAt.getTime()) / 60000;
      if (prior + live >= MAX_CONTINUOUS_WORK_MINUTES) {
        dirty = true;
        break;
      }
    }
  ...
  const stillOpen = await prisma.workSession.findFirst({
  ...
  return { reconciled: dirty, running: !!stillOpen };
}
...
export async function buildPlan(user, key, settingsInput) {
  ...
  // 2. Open tasks land on today's plan. Repeating ones wait until their due day.
  if (settings.planFromTasks) {
    ...
    for (const task of tasks) {
      ...
      if (!belongsOnTodayPlan(task, key)) continue;
      order += 1;
...
export async function spawnNextOccurrence(task) {
  ...
  if (repeat === 'NONE' || !task.dueDate) return null;
  const seriesId = task.seriesId || task.id;
  if (task.repeatCount) {
    const seen = await prisma.task.count({ where: { OR: [{ id: seriesId }, { seriesId }] } });
    if (seen >= task.repeatCount) return null;
  }
  const next = nextDueDate(task.dueDate, repeat, {
  ...
  if (!task.seriesId) {
    await prisma.task.update({ where: { id: task.id }, data: { seriesId } });
  }
...
export async function maybeSpawnOnComplete(before, after) {
  if (!after || after.status !== 'COMPLETED') return null;
  if (before?.status === 'COMPLETED') return null;
  return spawnNextOccurrence(after);
}
...
export async function setPointDone(userId, pointId, done) {
  ...
  if (point.taskId) {
    const existing = await prisma.task.findUnique({ where: { id: point.taskId } });
    ...
    if (done) await maybeSpawnOnComplete(existing, after);
  }
  return updated;
}
