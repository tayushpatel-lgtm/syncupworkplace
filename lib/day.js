 ...
 */
async function abuttingPriorWorkMinutes(db, userId, session) {
  ...
  for (const prev of recent) {
    ...
    if (prev.kind === 'WORK') minutes += closedMinutes(prev.startedAt, prev.endedAt);
...
export async function dayTotals(userId, key) {
  ...
  if (totals.running) {
    ...
    if (open) {
      totals.priorWork = Math.round(await abuttingPriorWorkMinutes(prisma, userId, open));
    }
  }
  return totals;
}
